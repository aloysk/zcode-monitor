...
> 注：下文各 WP 的「现状锚点」均为 **2026-09-22 开工前快照**（如「仓库无测试目录」「log-tail 为纯轮询」），实施完成后不回改；当前事实以代码与 `docs/acceptance/residuals.md` 为准。
...
- **A0-2** `[命令]` Given 任意非仓库根 cwd（如 `%TEMP%`），When `node --test "F:/project/zcode-monitor-plan/test"`（绝对路径；注意 `npm test` 从子目录调用时 npm 会把 cwd 切回包根，故 cwd 无关性必须用直接调用 `node --test` 验证），Then 结果与 A0-1 一致（全绿），证明测试文件与 fixture 构建器不依赖进程工作目录。修订注记 2026-09-23：初稿按计划的 `tests/` 目录布局书写，实施落地为 `test/`（聚合入口 `test/index.js`，commit 3c1e8f7 使 `node --test <目录>` 形态在 Node v24 可用）后本条未同步——现已按实际目录修订，修订后字面命令实测通过（exit 0）。
...
- **A0-4** `[测试]` Given fixture SQLite 库（含 `model_usage` / `tool_usage` / `session` / `turn_usage` / `message` / `part` 表与少量构造行），When 以注入路径调用 `db.js` 导出的每个查询函数——**查询函数清单固定为 25 个**（按 `server/db.js` 的 `module.exports` 导出面排除——行号易漂移、以导出面为准——排除 `DB_PATH` / `LOG_DIR` / `ROLLOUT_DIR` / `db` / `warmDb` / `invalidateDb` / `ts` / `j` / `startOfDayMs` 后点得：overviewKpis、timeseries、breakdownByModel、breakdownByTool、overviewSpeed、recentSpeed、completedSince、todayUsage、sessionList、sessionGet、sessionTurns、sessionConversation、sessionActivity、sessionChildren、sessionReasoning、errorsList、errorSummary、slowTools、recentModelRows、recentToolRows、latestModelStartedAt、latestToolStartedAt、agentsForest、recentToolRowsAfterRowid、latestToolRowid；WP2 触碰导出面时清单同步修订。修订注记 2026-09-23：初稿 23 个；livegen 工具失败水位（T5）新增 `recentToolRowsAfterRowid` / `latestToolRowid` 后增至 25——这两个函数的行形状与 rowid 水位语义由 test/livegen-error.test.js 集成覆盖，db-smoke 的逐函数跑通守护已同步补入），Then 全部不抛错且返回值字段结构与现行调用方期望一致。抽查断言集固定为：`overviewKpis().tokens.input` 为数值、`timeseries()` 为数组且元素含 `bucket` / `calls`、`sessionList()[].total_tokens` 为数值或 null、`agentsForest()` 含 `roots` 数组。
...
- **A3-3** `[命令]` Given `server/log-tail.js` 与 `tools/log-latency-probe.js`（零写入承诺的服务端主体），When `grep -rnE 'writeFile|appendFile|createWriteStream|open(Sync)?\([^)]*['\''"]((w|wx|a|ax)\+?|r\+|rs\+)' server/log-tail.js tools/log-latency-probe.js`，Then 无匹配（grep 退出码 1 属预期——ERE 不支持前向查找，故枚举写打开模式而非排除 `r`；模式以单引号承载、内嵌引号经 `'\''` 转义，字面可执行——2026-09-22 实测：`bash -n` 语法通过；对现存 `server/log-tail.js` 退出码 1；`w/a/r+/wx/w+/a+/rs+/ax` 八种写打开形态全部命中、`r`/`rs` 不命中；`open(` 同时覆盖 `fs.open` / `fs.openSync` / `fs.promises.open`；动态面由 A0-3 的 tmpdir 注入守护兜底）。修订注记 2026-09-23（与实施计划同步）：范围由初稿的 `server/log-tail.js tests/` 收窄为 `server/log-tail.js tools/log-latency-probe.js`——测试目录合法含写 tmpdir fixture 的 `fs.writeFileSync` / `fs.appendFileSync`（与 A3-1 要求测试向 fixture 追加行自相矛盾），纳入则本门禁恒命中、退出码 0、不可执行；收窄理由已在实施计划 T6 Step 5 留痕（docs/plans/ecosystem-adoption-v1.md）。
