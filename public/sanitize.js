...
(function (root, factory) {
  ...
  const RULES = [
    ...
    // data:/javascript: 内联 URI（URL 规则不含这两协议，单列；停止集同路径规则含全角标点）
    { re: new RegExp('\\b(?:data|javascript):[^' + PUNCT + ']+', 'gi'), sub: '[链接]' },
    ...
    // 密钥样式·sk- 前缀 token。锚不用 \b/lookbehind：'keysk-…' 粘连时 \b 在词中
    // 失配、整体漏过——从 'sk-' 起剥除即可（残前缀不是密钥材料）。体内容忍单
    // 个空格（跨行分段凭据，空白已先行收敛为单空格；空格后须仍是 token 字符，
    // 尾部不吃空格）。代价：英文行文里含 'sk-' 的连字符词（task-…）可能被整体
    // 收敛——隐私闸宁过杀，气泡文本以中文/数值为主，可接受。
    { re: /sk-[A-Za-z0-9_-](?:[A-Za-z0-9_-]|\s(?=[A-Za-z0-9_-])){5,}/g, sub: '[密钥]' },
