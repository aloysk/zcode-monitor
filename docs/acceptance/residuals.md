...
| R-9 | overview 回退路径「慢但可用」（R1 低-4 驳回登记） | **已知取舍** | DB 聚合不可用时逐 JSONL 现算，可能秒级耗时。R1 低-4 的 TTL 缓存建议被驳回（回退是异常路径、低频；缓存会掩盖恢复时刻的陈旧窗口，正确性优先于回退速度）。接受残余：触发回退期间该端点慢，但可用性与新鲜度正确。 |
| R-10 | log-tail 换名回归丢行（R2 低-c） | **已销账（R2）** | `readOffsets` Map（名字→语义未读偏移，截断取 min）替代「读过」集合：回归到已读文件从记录偏移续读，锚定后新写入不丢。测试：`test/log-tail.test.js` A3-8。有意保留的窄窗取舍：删除/复活（高-B、低-d）按「复活即当刻 size 补锚定」，复活瞬间已存在的行不追认——避免整文件回放，代码注释逐处标明。 |
