...
(function (root, factory) {
  ...
  const RULES = [
    ...
    // 密钥样式·Google key（AIza + 恰 35 位续段，总 39 字符）：39 < 长 base64
    // 规则的 40 位下限——专用规则补位（R3 评审实测漏剥）。前缀 AIza 是 Google
    // 官方形态（大小写敏感），不加 i 旗
    { re: /AIza[0-9A-Za-z_-]{35}/g, sub: '[密钥]' },
    // R3 补则（同形态扩展）：+rk_（Stripe restricted key）、i 旗（SK_LIVE_/
    // sk_Live_ 混合大小写同形态）、体内容忍单空格续段（对齐 sk- 规则——空格
    // 分段凭据在收敛后的单行语境仍是同一条；空格后须仍是密钥字符，不吃尾空格）
    { re: /(?:sk|pk|rk)_(?:(?:live|test)_)?[A-Za-z0-9](?:[A-Za-z0-9]|\s(?=[A-Za-z0-9])){15,}/gi, sub: '[密钥]' },
    ...
    // 均带小数点/单位不受影响）
    // R3 补则：分段分隔符类由单空格扩为 [ :.-]——冒号分段（aa:bb:… 的 MD5/
    // SSH 指纹形态，32 个 hex 字符两两分段）与点分/连字符分段同属该形态，旧
    // 规则只容忍空格而漏剥。过杀面：纯数字/十六进制字母词以 . - 分段合计
    // ≥32 也会收敛（宁过杀，同上取舍）
