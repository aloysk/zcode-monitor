...
「实时监控」页常驻**快照绊线卡**，顶栏在告警态亮出「快照活动!」红标（语义见 `server/snapshot-watch.js` 头注）：boot 时的目录状态即零点，此后任何新增工作区、`state.json` 重写、pending 加密工件增减都判定为机制复活并闩锁告警；fs.watch（Windows/macOS 递归）秒级快路径 + 30s 轮询兜底；目录不可读（读取被拒）如实显示「目录不可读」态而不是伪装成「静默」，单次扫描被截断（目录异常大）或部分内容读不了时该拍不参与差异判定并在卡上如实标注。已知边界：绊线只在面板运行时段设防，面板停机期间的活动以启动后的「遗留静止」态呈现（可用最后活动时间辅助判断）；面板重启会重置零点。
...
锁定后 ZCode 写不进该目录、快照上传链路失效，代价是「检查点回滚 / 时间线」功能不可用。该锁定只拒写入/创建、**不拒读取**——绊线卡照常扫描，保持「静默 / 遗留静止」态（目录内容冻结）；只有连读取一并拒绝的更激进锁定才会让绊线卡转「目录不可读」态（读取被拒的显性信号，非本程序所必需）。若锁定后目录里仍有历史快照残留且介意其存在，可先自行确认后手动清理再锁定。注意：锁定/解除是**对 `~/.zcode/` 的写入性操作**，由你手动执行——本面板自身对 `~/.zcode/` 始终零写入，不提供也代行不了这个动作。
...
安全姿态（与隐私相关的部分）：面板无鉴权、默认只绑 `127.0.0.1`，全部 `/api` 仅接受回环 Host（`127.0.0.1` / `localhost`，防 DNS rebinding 整库转录）；全站下发 CSP 与 `X-Content-Type-Options: nosniff`，前端零外联（Chart.js 已本地化到 `public/assets/`，字体走系统栈——R-8 已销账，见 `docs/acceptance/residuals.md`）；`/pets` 静态目录内非图片一律强制下载，导入夹带的页面类文件无法以面板同源执行。手动 checkpoint 的拒绝语义：WAL 近 60s 内有写入 → `409 wal_active`（`?force=1` 也不越过，绝不与真实写入方抢锁）；探测显示 ZCode 运行中 → `409 zcode_running`（`?force=1` 可越过）；锁竞争 → `503 checkpoint_busy`（可重试）；`?force=1` 另要求请求头 `X-Zcode-Monitor-Checkpoint: 1`（面板按钮自动携带；防跨站简单请求触发，缺头 → `403`）。`POST /api/restart`（桌宠菜单「重启面板」）同款首部闸（`X-Zcode-Monitor-Restart: 1`，缺头 → `403`），POST-only；同步 spawn 失败如实 500，异步失败/接替早夭则撤销重启（旧进程不退出，失败落 `logs/restart-child.log`）。注意：首部闸只防跨站请求；面板自身页面中的脚本（如发生 XSS）仍可触发重启，属可用性级影响（仅本机服务短暂中断），无数据面。全部 `/api` 行数参数（limit/max/offset）统一钳界，负值不再构成无上限查询。
