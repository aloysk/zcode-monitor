...
router.get('/:table', (req, res) => {
  ...
  // limit/offset 双侧钳界（R4 修-high）：`Math.min(+q.limit || 100, 1000)` 对
  // ?limit=-1 产出 -1，SQLite 的负 LIMIT = 无上限 → SELECT * 整表同步物化
  //（message 72万行实测事件循环冻结）。下界钳 1/0 后：负值与 NaN（?limit=abc）
  // 都回落缺省或安全值，上限 1000 不变。
  const limit = Math.max(1, Math.min(+req.query.limit || 100, 1000));
